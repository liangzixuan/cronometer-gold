...
output never enter the diagnostic. PowerShell version discovery has one
twenty-second bound; WSL path conversion retains its five-second bound, and each
synthetic collector process retains its twenty-second bound. The version-only
startup allowance follows the exact hosted `powershell-version-timeout` diagnostic
on September 10, 2026, before any collector phase ran. It changes no version,
identity, output, golden-data or negative-case acceptance condition. A hung version
probe can now take up to fifteen additional seconds to fail. There is no warm-up,
retry or skipped assertion. A failure must still be diagnosed from its exact stage;
a generic failure or slow runner alone does not authorize further changes to
