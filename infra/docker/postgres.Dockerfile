...
# Upgrade the Alpine OpenSSL and util-linux runtime packages implicated by the
# hosted strict scans. The runtime is never root, so the official entrypoint's
# gosu branch is unreachable and unnecessary. PGDATA and both runtime tmpfs
# mounts must be presented pre-owned by 70:70; these image paths make
...
RUN set -eux; \
    ...
    apk list --installed libuuid | grep -Fx 'libuuid-2.42.3-r0 aarch64 {util-linux} (BSD-3-Clause) [installed]'; \
