...
ARG POSTGRES_IMAGE=docker.io/library/postgres:17.11-alpine3.24@sha256:18cfe3ef5e6815560c98237d6216d1e5119702fb0f3894c8785dd58b8bbe5d73
...
# gosu is the only HIGH/CRITICAL-bearing file in the reviewed ARM64 parent.
# The runtime is never root, so the official entrypoint's gosu branch is both
# unreachable and unnecessary. PGDATA and both runtime tmpfs mounts must be
# presented pre-owned by 70:70; these image paths make Docker-created volumes
# inherit the same ownership.
RUN set -eux; \
