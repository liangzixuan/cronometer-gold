...
RUN --mount=type=cache,id=nutrition-pnpm-worker-v1,target=/pnpm/store,sharing=locked \
    test "$(pnpm --version)" = 11.19.0 && \
    pnpm install --frozen-lockfile --strict-peer-dependencies --filter @nutrition-tracker/worker... && \
...
COPY --from=build --chown=1000:1000 /opt/deploy/worker/ ./
...
HEALTHCHECK --interval=60s --timeout=10s --start-period=30s --retries=3 \
  CMD ["/nodejs/bin/node", "dist/database-readiness-probe.js"]
