...
RUN --mount=type=cache,id=nutrition-pnpm-api-v1,target=/pnpm/store,sharing=locked \
    test "$(pnpm --version)" = 11.19.0 && \
    pnpm install --frozen-lockfile --strict-peer-dependencies --filter @nutrition-tracker/api... && \
...
LABEL io.cronometer.runtime.component="api" \
...
COPY --from=build --chown=1000:1000 /opt/deploy/api/ ./
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3 \
  CMD ["/nodejs/bin/node", "-e", "fetch('http://127.0.0.1:' + (process.env.API_PORT || '3001') + '/ready').then((response) => { if (!response.ok) process.exit(1) }).catch(() => process.exit(1))"]
