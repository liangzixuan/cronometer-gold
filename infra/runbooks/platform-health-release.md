...
The signed v6 `physicalDeviceApiRelay` must also pin the exact reviewed `.ts.net`
...
An HTTPS-shaped URL alone is not evidence. The same v6 manifest must bind the
...
internal artifacts. The independently reviewed v6 manifest binds four distinct
