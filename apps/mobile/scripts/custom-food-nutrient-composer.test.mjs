...
describe("native named custom-food nutrient composer", () => {
  it("exposes every loaded name/unit, filters locally and leaves all drafts untouched until Add", async () => {
    ...
    try {
      ...
      expect(text(tree)).toContain("1 matching of 31 loaded nutrients");
      expect(requests).toHaveLength(before);
      tree = await type(harness, "Find an available nutrient by name", "no such nutrient");
      expect(text(tree)).toContain("0 matching of 31 loaded nutrients");
      tree = await click(harness, "Clear nutrient filter");
      expect(input(tree, "Find an available nutrient by name").props.value).toBe("");
      expect(text(tree)).toContain("31 matching of 31 loaded nutrients");
      const choices = nodes(
        tree,
        (node) =>
          node.props.accessibilityRole === "radio" &&
          text(node).startsWith("Available nutrient ") &&
          text(node).endsWith(" (g)"),
      );
      expect(choices.map((choice) => [choice.key, text(choice)])).toEqual(
        registry.map((row) => [row.id, `${row.name} (${row.unit})`]),
      );
      expect(button(tree, "Available nutrient 30 (g)").props.accessibilityState.selected).toBe(
        true,
      );
      ...
      button(tree, "Clear nutrient filter").props.onPress();
  ...
  it("wraps complete maximum-length named nutrient labels and adds the selected exact ID", async () => {
    ...
    try {
      ...
      tree = await type(harness, "Find an available nutrient by name", "no such nutrient");
      expect(text(tree)).toContain("0 matching of 3 loaded nutrients");
      tree = await click(harness, "Clear nutrient filter");
      expect(input(tree, "Find an available nutrient by name").props.value).toBe("");
  ...
  for (const amount of ["0", "0.00000000000000100"])
    it(`appends exact quantified ${amount} losslessly and saves only explicitly`, async () => {
      ...
      try {
        ...
        expect(canonical(tree)).toBe(raw);
        expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe(amount);
  ...
  for (const [label, reason] of [
    ...
    it(`keeps ${label} distinct and requires an explicit unknown reason`, async () => {
      ...
      try {
        ...
        expect(writes(requests)).toHaveLength(0);
        await type(harness, "Find an available nutrient by name", "no matching nutrient");
        await click(harness, "Create private food");
        expect(JSON.parse(writes(requests)[0].body).nutrients).toEqual([
          {
            nutrientId: "203",
            state: reason ? "unknown" : "trace",
  ...
  it("blocks duplicate or invalid manual text before Add and preserves unresolved entry until explicit recovery", async () => {
    const { harness, requests } = setup();
    try {
      await type(harness, "Name", "Owner food");
      await click(harness, "Protein (g)");
      await click(harness, "Trace");
      let tree = await type(harness, "Canonical nutrients per 100 g", "203=1");
      expect(button(tree, "Protein (g) · already in this draft").props.disabled).toBe(true);
      expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
      let stateWrites = harness.stateWrites;
      button(tree, "Add nutrient row to draft").props.onPress();
      button(tree, "Protein (g) · already in this draft").props.onPress();
      expect(harness.stateWrites).toBe(stateWrites);
      ...
      expect(writes(requests)).toHaveLength(0);
      tree = await type(harness, "Canonical nutrients per 100 g", "broken manual text");
      expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
  ...
  it("fences stale manual/choice/Add/Save/Revise/Cancel controls and keeps same-value edits usable", async () => {
    ...
    try {
      ...
      tree = await type(harness, "Find an available nutrient by name", "prot");
  ...
  it("holds exact body/key through malformed success and A-to-B-to-A retries", async () => {
    ...
    try {
      ...
      await type(harness, "Find an available nutrient by name", "no-match");
      const tree = await click(harness, "Create private food");
      expect(input(tree, "Name").props.value).toBe("Owner food");
      const operation = hooks.operation;
      await click(harness, "Clear nutrient filter");
      expect(hooks.operation).toBe(operation);
      await click(harness, "Create private food");
    ...
    for (const phase of ["fetch", "JSON"])
      it(`fences ${phase} receipt and old private controls through ${boundary} replacement`, async () => {
        ...
        try {
          ...
          const oldEdit = draftEdit(tree, "208").props.onPress;
          await click(harness, "Create private food");
          harness.updateProps(
            boundary === "owner"
  ...
  it("rejects a retained Revise callback after a same-scope list refresh installs a newer row", async () => {
    ...
    try {
      let tree = await type(harness, "Find an available nutrient by name", "prot");
      const oldClear = button(tree, "Clear nutrient filter").props.onPress;
      const oldEntryClear = button(tree, "Clear nutrient entry").props.onPress;
  ...
  for (const boundary of ["background", "replay"])
    it(`retains uncertain save identity across ${boundary} and rejects its late401`, async () => {
      ...
      try {
        await fillManual(harness, "0.0100");
        await click(harness, "Protein (g)");
        await click(harness, "Trace");
  ...
  it("ignores delayed JSON and retained callbacks after unmount without custom state writes", async () => {
    ...
    await fillManual(harness);
    const tree = await type(harness, "Find an available nutrient by name", "prot");
    const old = input(tree, "Name").props.onChangeText;
    const oldClear = button(tree, "Clear nutrient filter").props.onPress;
    const oldEntryClear = button(tree, "Clear nutrient entry").props.onPress;
    const oldRemove = draftRemove(tree, "208").props.onPress;
  ...
  for (const uncertain of [null, "unknown", "extension"])
    it(`requires exact active AppState instead of ${uncertain}`, async () => {
      ...
      try {
        ...
        tree = await click(harness, "Refresh private data");
        expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
        expect(button(tree, "Protein (g)").props.disabled).toBe(false);
        tree = await click(harness, "Protein (g)");
  ...
  for (const outcome of ["same", "removed", "failed"])
    it(`invalidates at full refresh start and handles ${outcome} metadata without silently substituting a selection`, async () => {
      ...
      try {
        ...
        const oldNone = button(trendSection(tree), "None").props.onPress;
        refreshing = true;
        button(tree, "Refresh private data").props.onPress();
        oldLoad();
        {
          const noneWrites = harness.stateWrites;
          oldNone();
          expect(harness.stateWrites).toBe(noneWrites);
        }
        oldFilter("late");
        tree = await harness.settle();
...
describe("native optional biometric trend None", () => {
  for (const status of ["active", "archived"]) {
    it(`offers None before ${status} metric choices and restores paired reads explicitly`, async () => {
      const metric = { ...otherTrendDefinition, status };
      const { harness, requests } = setupTrends(undefined, { metrics: [trendDefinition, metric] });
      try {
        await setTrendDates(harness);
        let tree = await click(harness, "Load local-day trends");
        const nutrientHeader = trendHeaders(tree)[0];
        const rows = text(trendSection(tree));
        const count = requests.length;
        const operation = hooks.operation;
        const dates = trendDateValues(tree);
        tree = await clickTrend(harness, "None");
        expect(button(trendSection(tree), "None").key).toBe("");
        expect(button(trendSection(tree), "None").props.accessibilityRole).toBe("radio");
        expect(button(trendSection(tree), "None").props.accessibilityState).toEqual({
          selected: true,
          disabled: false,
        });
        const choices = nodes(
        ...
        expect(text(trendSection(tree))).toContain("2026-11-01 : 0 g · exact");
        expect(trendDateValues(tree)).toEqual(dates);
        expect(requests).toHaveLength(count);
        expect(hooks.operation).toBe(operation);
        tree = await click(harness, "Load local-day trends");
        expect(
        ...
        expect(
          trendRequests(requests)
            .slice(3)
            .map((request) => [request.url.pathname, request.url.searchParams.get("definitionId")]),
        ).toEqual([
          ["/v1/trends/nutrients", null],
          ["/v1/trends/biometrics", metric.id],
        ]);
        expect(trendHeaders(tree)).toHaveLength(2);
        expect(writes(requests)).toHaveLength(0);
      } finally {
  ...
  it("offers selected None with verified-empty definitions and disables Load if nutrients are also empty", async () => {
    const { harness, requests } = setupTrends(undefined, { available: [], metrics: [] });
    try {
      const tree = await setTrendDates(harness);
      ...
      button(tree, "Load local-day trends").props.onPress();
      expect(harness.stateWrites).toBe(stateWrites);
      expect(requests).toHaveLength(count);
    } finally {
      harness.unmount();
  ...
  it("clears the only selected series locally without requesting an empty trend", async () => {
    const { harness, requests } = setupTrends(undefined, { available: [] });
    try {
      await setTrendDates(harness);
      await click(harness, "Load local-day trends");
  ...
  for (const outcome of ["loaded", "pending", "failed"]) {
    it(`keeps matching None a true no-op with ${outcome} nutrient-only results`, async () => {
      const held = deferred();
      let captured;
      const { harness, requests } = setupTrends((request) => {
        if (request.url.pathname !== "/v1/trends/nutrients") return undefined;
      ...
      try {
        ...
        if (outcome === "pending") {
          expect(captured.signal.aborted).toBe(false);
          held.resolve(response(nutrientTrendResponse(captured)));
          expect(trendHeaders(await harness.settle())).toHaveLength(1);
        }
      } finally {
        harness.unmount();
      }
  ...
  it("retains None through presets and same-private removed/reintroduced metric refreshes", async () => {
    historyClock("2026-11-07T18:00:00.000Z");
    let metrics = [trendDefinition, otherTrendDefinition];
    const { harness, requests } = setupTrends((request) =>
      request.url.pathname === "/v1/biometrics/definitions"
    ...
    try {
      ...
      await clickTrend(harness, "None");
      await type(harness, "Find a trend nutrient by name", "  prot  ");
      await type(harness, "Name", "  Raw custom food  ");
      for (const days of [7, 30, 90]) {
        const tree = await click(harness, `Last ${days} days`);
        expect(button(trendSection(tree), "None").props.accessibilityState.selected).toBe(true);
        expect(input(tree, "Find a trend nutrient by name").props.value).toBe("  prot  ");
      }
      const dates = trendDateValues(await harness.settle());
      for (const next of [[], [otherTrendDefinition, trendDefinition]]) {
        metrics = next;
        const tree = await click(harness, "Refresh private data");
        expect(button(trendSection(tree), "None").props.accessibilityState.selected).toBe(true);
        expect(trendDateValues(tree)).toEqual(dates);
        expect(input(tree, "Find a trend nutrient by name").props.value).toBe("  prot  ");
        expect(input(tree, "Name").props.value).toBe("  Raw custom food  ");
      }
      expect(trendRequests(requests)).toHaveLength(0);
      harness.updateProps({ ownerUserId: otherOwner, accessToken: "new-private-scope" });
      const tree = await harness.settle();
      expect(button(trendSection(tree), "None").props.accessibilityState.selected).toBe(false);
      expect(
        button(
  ...
  it("preserves exact raw editors, history, selected nutrient and search when clearing the biometric", async () => {
    const { harness, requests, props } = setupReadings();
    try {
      await setTrendDates(harness);
      await clickTrend(harness, "Sodium · mg");
      await type(harness, "Find a trend nutrient by name", "  sod  ");
      await pressBiometric(harness, "Edit", readingEvent.id);
      await type(harness, readingValueLabel(), "-71.00000000100");
      await type(harness, "Definition notes", "  Raw metric notes  ");
      await type(harness, "Private in-app label", "  Raw reminder  ");
      await type(harness, "Name", "  Raw custom food  ");
      await type(
        harness,
      ...
      input(logEditor(await harness.settle()), "Local time").props.onChangeText("12:34");
      const before = await harness.settle();
      const inputs = editorSnapshot(before).inputs;
      const history = text(biometricSection(before));
      const count = requests.length;
      const operation = hooks.operation;
      const tree = await clickTrend(harness, "None");
      expect(editorSnapshot(tree).inputs).toEqual(inputs);
      expect(text(biometricSection(tree))).toBe(history);
      expect(button(trendSection(tree), "Sodium · mg").props.accessibilityState.selected).toBe(
        true,
      );
      expect(requests).toHaveLength(count);
  ...
  for (const phase of ["success", "fetch401", "JSON", "failure"]) {
    it(`retires the paired read on None and rejects late biometric ${phase} after a later nutrient load`, async () => {
      const held = deferred();
      const later = deferred();
      let biometricRequest;
      let nutrientRequest;
      let attempts = 0;
      const { harness, requests, props } = setupTrends((request) => {
        if (request.url.pathname === "/v1/trends/biometrics") {
        ...
        if (request.url.pathname === "/v1/trends/nutrients" && ++attempts === 2) {
      ...
      try {
        ...
        await click(harness, "Load local-day trends");
        const count = requests.length;
        let tree = await clickTrend(harness, "None");
        expect(biometricRequest.signal.aborted).toBe(true);
        expect(requests).toHaveLength(count);
        expect(trendHeaders(tree)).toHaveLength(0);
        await click(harness, "Load local-day trends");
        held.resolve(
          phase === "fetch401"
            ...
            : phase === "failure"
              ? response({}, 503)
              : phase === "JSON"
                ? biometricTrendResponse(biometricRequest)
                : response(biometricTrendResponse(biometricRequest)),
        );
        tree = await harness.settle();
        expect(button(tree, "Loading…").props.disabled).toBe(true);
        expect(nutrientRequest.signal.aborted).toBe(false);
        expect(props.onUnauthorized).not.toHaveBeenCalled();
        later.resolve(response(nutrientTrendResponse(nutrientRequest)));
        tree = await harness.settle();
        expect(trendHeaders(tree)).toHaveLength(1);
        expect(trendRequests(requests)).toHaveLength(3);
      } finally {
  ...
  it("does not let an obsolete paired read unlock a current custom save after None", async () => {
    const old = deferred();
    const save = deferred();
    const { harness, requests } = setupTrends((request) => {
      if (request.method === "POST") return save.promise;
      if (request.url.pathname === "/v1/trends/biometrics") return old.promise;
      return undefined;
    });
    try {
      await setTrendDates(harness);
      ...
      await clickTrend(harness, "None");
      await click(harness, "Create private food");
      old.resolve(response({}, 503));
      const tree = await harness.settle();
      expect(button(tree, "Create private food").props.disabled).toBe(true);
      expect(writes(requests)[0].signal.aborted).toBe(false);
      save.resolve(receipt(writes(requests)[0]));
      await harness.settle();
    } finally {
      harness.unmount();
  ...
  it("preserves a live ambiguous custom retry body/key and leaves protected queues untouched", async () => {
    const held = deferred();
    let attempts = 0;
    const { harness, requests, props } = setupTrends((request) => {
      if (request.method === "POST" && request.url.pathname === "/v1/custom-foods") {
  ...
  it("rejects retained None, metric and Load callbacks after raw ABA and synchronous series changes", async () => {
    const { harness, requests } = setupTrends();
    try {
      let tree = await setTrendDates(harness);
      const oldNone = button(trendSection(tree), "None").props.onPress;
...
describe("native named nutrient filter Clear", () => {
  it("preserves selected raw amount, trace/unknown state and reason until explicit Add", async () => {
    const { harness, requests, props } = setup();
    try {
      const raw = " 208=125.5000\r\n\r\n 999=unknown:not_reported  ";
      await type(harness, "Name", "  Exact draft name  ");
      await type(harness, "Canonical nutrients per 100 g", raw);
      await click(harness, "Protein (g)");
      await type(harness, "Protein amount (g per 100 g)", "0.00000000000000100");
      await click(harness, "Unknown");
      ...
      const count = requests.length;
      const operation = hooks.operation;
      let tree = await click(harness, "Clear nutrient filter");
      ...
      expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe("0.00000000000000100");
      tree = await click(harness, "Unknown");
      expect(button(tree, "Withheld").props.accessibilityState.selected).toBe(true);
      expect(input(tree, "Name").props.value).toBe("  Exact draft name  ");
      expect(canonical(tree)).toBe(raw);
      expect(requests).toHaveLength(count);
      expect(hooks.operation).toBe(operation);
      expect(props.quickAddOutboxController.enqueueOperation).not.toHaveBeenCalled();
...
describe("native unfinished nutrient entry Save protection", () => {
  const blocked =
    "Add the unfinished nutrient row to the draft or choose Clear nutrient entry before saving.";
  for (const amount of [" ", "not-an-exact-amount"])
    it(`retains invalid raw entry ${JSON.stringify(amount)} through failed Add and Save until explicit Clear`, async () => {
      const { harness, requests, props } = setup();
      try {
        ...
        const operation = hooks.operation;
        let tree = await click(harness, "Add nutrient row to draft");
        expect(canonical(tree)).toBe("208=0.00000000000000100");
        tree = await click(harness, "Create private food");
        expect(text(tree)).toContain(blocked);
        expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe(amount);
        await click(harness, "Clear nutrient filter");
        tree = await click(harness, "Create private food");
        expect(text(tree)).toContain(blocked);
        const fields = rawCustomFields(tree);
        tree = await click(harness, "Clear nutrient entry");
        expect(rawCustomFields(tree)).toEqual(fields);
        expect(input(tree, "Exact amount per 100 g").props.value).toBe("");
        expect(button(tree, "Protein (g)").props.accessibilityState.selected).toBe(false);
        expect(text(tree)).not.toContain(blocked);
        expect(requests).toHaveLength(before);
        expect(hooks.operation).toBe(operation);
        expect(props.quickAddOutboxController.enqueueOperation).not.toHaveBeenCalled();
        expect(props.quickAddOutboxController.requestDrain).not.toHaveBeenCalled();
  ...
  for (const entry of ["selection", "trace", "unknown", "hidden raw amount"])
    it(`protects ${entry} work without requiring a valid appended candidate`, async () => {
      const { harness, requests } = setup();
      try {
        await fillManual(harness);
        if (entry === "selection" || entry === "hidden raw amount")
          await click(harness, "Protein (g)");
        if (entry === "hidden raw amount")
          await type(harness, "Protein amount (g per 100 g)", "  raw  ");
        if (entry === "trace" || entry === "hidden raw amount") await click(harness, "Trace");
        if (entry === "unknown") await click(harness, "Unknown");
        let tree = await click(harness, "Create private food");
        expect(text(tree)).toContain(blocked);
        expect(writes(requests)).toHaveLength(0);
        expect(canonical(tree)).toBe("208=0");
        if (entry === "hidden raw amount") {
          tree = await click(harness, "Quantified");
          expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe("  raw  ");
        }
      } finally {
        harness.unmount();
      }
  ...
  it("preserves unresolved entry through same-private failed registry refresh and allows entry Clear without that registry", async () => {
    let failed = false;
    const { harness, requests } = setup((request) =>
      failed && request.url.pathname === "/v1/nutrients/targetable" ? response({}, 503) : undefined,
    );
    try {
      await fillManual(harness, "12.340006");
      await type(harness, "Notes", "  raw notes  ");
      await click(harness, "Protein (g)");
      await type(harness, "Protein amount (g per 100 g)", "0.00000000000000100");
      await click(harness, "Unknown");
      await click(harness, "Withheld");
      ...
      expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
      expect(button(tree, "Clear nutrient filter").props.disabled).toBe(true);
      expect(button(tree, "Clear nutrient entry").props.disabled).toBe(false);
      expect(button(tree, "Withheld").props.accessibilityState.selected).toBe(true);
      const before = requests.length;
      const fields = rawCustomFields(tree);
      tree = await click(harness, "Create private food");
      ...
      await click(harness, "Create private food");
      expect(JSON.parse(writes(requests)[0].body).nutrients).toEqual([
        { nutrientId: "208", state: "quantified", amountPer100Grams: "12.340006" },
      ]);
    } finally {
      harness.unmount();
    }
  ...
  it("compares the acknowledged exact entry rather than query or edit history, including hidden reason changes", async () => {
    const { harness, requests } = setup();
    try {
      await fillManual(harness);
      await click(harness, "Protein (g)");
      ...
      await click(harness, "Withheld");
      await click(harness, "Add nutrient row to draft");
      await click(harness, "Not reported");
      await click(harness, "Trace");
      let tree = await click(harness, "Create private food");
      expect(text(tree)).toContain(blocked);
      await click(harness, "Unknown");
      ...
      await click(harness, "Refresh private data");
      await click(harness, "Create private food");
      expect(JSON.parse(writes(requests)[0].body).nutrients[1]).toEqual({
        nutrientId: "203",
        state: "unknown",
        amountPer100Grams: null,
        reason: "withheld",
  ...
  for (const mode of ["create", "revise"])
    it(`keeps ${mode} pending ownership and exact ambiguous retry through blocked input and local Clear`, async () => {
      const held = deferred();
      let attempt = 0;
      const { harness, requests, props } = setup((request) =>
        request.method === "POST" ? (++attempt === 1 ? held.promise : receipt(request)) : undefined,
      ...
      try {
        if (mode === "revise") await click(harness, "Revise");
        else await fillManual(harness, "12.340006");
        await click(harness, "Protein (g)");
        await click(harness, "Trace");
        let tree = await click(harness, "Add nutrient row to draft");
        expect(
          button(tree, "Protein (g) · already in this draft").props.accessibilityState,
        ).toEqual({ selected: true, disabled: true });
        expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
        const acknowledgedWrites = harness.stateWrites;
        button(tree, "Protein (g) · already in this draft").props.onPress();
        button(tree, "Add nutrient row to draft").props.onPress();
        expect(harness.stateWrites).toBe(acknowledgedWrites);
        ...
        expect(first.url.pathname).toBe(
          mode === "revise" ? `/v1/custom-foods/${foodId}/revisions` : "/v1/custom-foods",
        );
        ...
        remove();
        edit();
        draftEdit(tree, "208").props.onPress();
        ...
        tree = await click(harness, label);
        expect(text(tree)).toContain(blocked);
        const bodyFields = rawCustomFields(tree);
        tree = await click(harness, "Clear nutrient entry");
        expect(rawCustomFields(tree)).toEqual(bodyFields);
        expect(input(tree, "Find an available nutrient by name").props.value).toBe("  sod  ");
        expect(requests).toHaveLength(count);
        expect(hooks.operation).toBe(operation);
        expect(props.quickAddOutboxController.enqueueOperation).not.toHaveBeenCalled();
        expect(props.quickAddOutboxController.requestDrain).not.toHaveBeenCalled();
        ...
        expect(input(tree, "Name").props.value).toBe("");
        await fillManual(harness, "0");
        await click(harness, "Create private food");
        expect(writes(requests)).toHaveLength(3);
      } finally {
        harness.unmount();
      }
    });

  it("keeps matching blank Clear harmless and fences retained Clear/Save and replacement callbacks around raw edits", async () => {
    const { harness, requests } = setup();
    try {
      await fillManual(harness);
      await type(harness, "Find an available nutrient by name", "  query  ");
      let tree = await copyCustom(harness);
      const discard = button(tree, discardCustomCopy).props.onPress;
      const oldClear = button(tree, "Clear nutrient entry").props.onPress;
      const oldSave = button(tree, "Create private food").props.onPress;
      let stateWrites = harness.stateWrites;
      oldClear();
      expect(harness.stateWrites).toBe(stateWrites);
      ...
      tree = await harness.settle();
      expect(input(tree, "Name").props.value).toBe(sourceFood.currentVersion.name);
      const amount = input(tree, "Exact amount per 100 g").props.onChangeText;
      amount("0");
      stateWrites = harness.stateWrites;
      oldClear();
      oldSave();
      expect(harness.stateWrites).toBe(stateWrites);
      tree = await harness.settle();
      expect(input(tree, "Exact amount per 100 g").props.value).toBe("0");
      tree = await requestCustomReplacement(harness, "revise");
      const revise = button(tree, discardCustomRevise).props.onPress;
      const blockedSave = button(tree, "Create private food").props.onPress;
      const clear = button(tree, "Clear nutrient entry").props.onPress;
      clear();
      stateWrites = harness.stateWrites;
      clear();
...
function draftRemove(tree, id) {
  ...
  const buttons = nodes(
    rows[0],
    (node) => node.type === "Pressable" && text(node).startsWith("Remove "),
  );
...
function draftEdit(tree, id) {
  const rows = nodes(draftNutrientSection(tree), (node) => node.type === "View" && node.key === id);
  expect(rows).toHaveLength(1);
  const buttons = nodes(
    rows[0],
    (node) => node.type === "Pressable" && text(node).startsWith("Edit "),
  );
  expect(buttons).toHaveLength(1);
  return buttons[0];
}
async function editDraftNutrient(harness, id) {
  const target = draftEdit(await harness.settle(), id);
  expect(target.props.disabled).toBe(false);
  target.props.onPress();
  return harness.settle();
}

describe("native fixed-target nutrient row editing", () => {
  const applyLabel = "Apply nutrient edit";
  const saveBlocked =
    "Apply the current nutrient edit or choose Clear nutrient entry before saving.";
  const stale =
    "The row or nutrient list changed. Choose Clear nutrient entry, then edit the current row.";
  for (const mode of ["create", "revise"])
    it(`applies an exact ${mode} row in place with other food edits and preserves the explicit ambiguous retry`, async () => {
      let attempt = 0;
      const { harness, requests, props } = setup((request) =>
        request.method === "POST"
          ? ++attempt === 1
      ...
      try {
        if (mode === "revise") await click(harness, "Revise");
        else await fillManual(harness);
        const raw = " 208=0\r\n\n 203=0.00 \n307=trace\r\n999=unknown:withheld  ";
        const expected = " 208=0\r\n\n203=0.00000000000000100\n307=trace\r\n999=unknown:withheld  ";
        let tree = await type(harness, "Canonical nutrients per 100 g", raw);
        const oldAdd = button(tree, "Add nutrient row to draft").props.onPress;
        tree = await editDraftNutrient(harness, "203");
        expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe("0.00");
        expect(button(tree, "Protein (g)").props.accessibilityState.selected).toBe(true);
        expect(button(tree, "Sodium (mg)").props.disabled).toBe(true);
        let stateWrites = harness.stateWrites;
        button(tree, "Sodium (mg)").props.onPress();
        oldAdd();
        expect(harness.stateWrites).toBe(stateWrites);
        await type(harness, "Find an available nutrient by name", "  no match  ");
        tree = await type(harness, "Protein amount (g per 100 g)", "0.00000000000000100");
        const oldApply = button(tree, applyLabel).props.onPress;
        await type(harness, "Name", "  Exact changed name  ");
        tree = await type(harness, "Notes", "  raw edited notes  ");
        stateWrites = harness.stateWrites;
        oldApply();
        expect(harness.stateWrites).toBe(stateWrites);
        const fields = rawCustomFields(tree);
        const count = requests.length;
        const operation = hooks.operation;
        tree = await click(harness, applyLabel);
        expect(rawCustomFields(tree)).toEqual({
          ...fields,
          "Canonical nutrients per 100 g": expected,
        });
        ...
        expect(props.quickAddOutboxController.enqueueOperation).not.toHaveBeenCalled();
        expect(props.quickAddOutboxController.requestDrain).not.toHaveBeenCalled();
        const save = mode === "revise" ? "Save new version" : "Create private food";
        await click(harness, save);
        const first = writes(requests)[0];
        expect(first.url.pathname).toBe(
          mode === "revise" ? `/v1/custom-foods/${foodId}/revisions` : "/v1/custom-foods",
        );
        ...
        expect(JSON.parse(first.body).nutrients).toEqual([
          { nutrientId: "208", state: "quantified", amountPer100Grams: "0" },
          { nutrientId: "203", state: "quantified", amountPer100Grams: "0.00000000000000100" },
          { nutrientId: "307", state: "trace", amountPer100Grams: null },
          { nutrientId: "999", state: "unknown", amountPer100Grams: null, reason: "withheld" },
        ]);
        await editDraftNutrient(harness, "203");
        tree = await click(harness, save);
        expect(text(tree)).toContain(saveBlocked);
        await click(harness, applyLabel);
        tree = await click(harness, save);
        expect(writes(requests)).toHaveLength(2);
        expect(writes(requests)[1].body).toBe(first.body);
        expect(writes(requests)[1].headers).toEqual(first.headers);
  ...
  it("keeps unchanged Apply byte-exact, repeats Edit as a true no-op, and reveals only accepted editing", async () => {
    const { harness, requests } = setup();
    try {
      await fillManual(harness);
      const raw = "\r\n \t203=0.00 \r\n\n307=trace\n";
      ...
      tree = await editDraftNutrient(harness, "203");
      expect(scrollTo).toHaveBeenCalledExactlyOnceWith({ y: 0, animated: true });
      const apply = button(tree, applyLabel).props.onPress;
      const stateWrites = harness.stateWrites;
      draftEdit(tree, "203").props.onPress();
      expect(harness.stateWrites).toBe(stateWrites);
  ...
  it("protects unfinished Add and active-edit scratch on switches, and Clear abandons only entry state", async () => {
    const { harness, requests } = setup();
    try {
      await fillManual(harness);
      const raw = "208=0\n203=trace\n307=0.00";
      await click(harness, "Protein (g)");
      await type(harness, "Protein amount (g per 100 g)", " raw unfinished ");
      await click(harness, "Unknown");
      await click(harness, "Withheld");
      await type(harness, "Canonical nutrients per 100 g", raw);
      let tree = await type(harness, "Find an available nutrient by name", "no match");
      const scrollTo = vi.fn();
      nodes(tree, (node) => node.type === "ScrollView")[0].props.ref.current = { scrollTo };
      tree = await editDraftNutrient(harness, "307");
      expect(text(tree)).toContain(
      ...
      await click(harness, "Clear nutrient entry");
      await editDraftNutrient(harness, "307");
      tree = await type(harness, "Sodium amount (mg per 100 g)", "still unfinished");
      const stateWrites = harness.stateWrites;
      draftEdit(tree, "307").props.onPress();
      expect(harness.stateWrites).toBe(stateWrites);
      tree = await editDraftNutrient(harness, "203");
      expect(text(tree)).toContain(
        "Apply the current nutrient edit or choose Clear nutrient entry before editing another row.",
      );
      expect(input(tree, "Sodium amount (mg per 100 g)").props.value).toBe("still unfinished");
      expect(scrollTo).toHaveBeenCalledTimes(1);
      tree = await requestCustomReplacement(harness, "revise");
      await click(harness, "Keep editing");
      tree = await requestCustomReplacement(harness, "copy");
      const discard = button(tree, discardCustomCopy).props.onPress;
      const fields = rawCustomFields(tree);
      tree = await click(harness, "Clear nutrient entry");
      discard();
  ...
  for (const change of ["manual edit/restore", "remove/re-add"])
    it(`keeps stale scratch and rejects retained or newly rendered Apply after ${change}`, async () => {
      const { harness, requests } = setup();
      try {
        await fillManual(harness);
        const raw = "208=0\r\n203=0.00\n";
        ...
        const apply = button(tree, applyLabel).props.onPress;
        if (change === "remove/re-add") await removeDraftNutrient(harness, "203");
        else await type(harness, "Canonical nutrients per 100 g", "invalid later text");
        tree = await type(harness, "Canonical nutrients per 100 g", raw);
        expect(button(tree, applyLabel).props.disabled).toBe(true);
        expect(text(tree)).toContain(stale);
        const stateWrites = harness.stateWrites;
        apply();
        button(tree, applyLabel).props.onPress();
        draftEdit(tree, "203").props.onPress();
        expect(harness.stateWrites).toBe(stateWrites);
        expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe("12.340006");
        expect(canonical(tree)).toBe(raw);
        await click(harness, "Clear nutrient entry");
        tree = await editDraftNutrient(harness, "203");
        expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe("0.00");
  ...
  for (const initial of ["named", "fallback"])
    it(`binds ${initial} edits to their receipt and permits current fallback ID edits without named metadata`, async () => {
      let available = initial === "named";
      const { harness, requests } = setup((request) =>
        request.url.pathname === "/v1/nutrients/targetable"
          ? available
            ? response({ data: [protein, sodium] })
            : response({}, 503)
          : undefined,
      );
      try {
        await fillManual(harness);
        await type(harness, "Canonical nutrients per 100 g", "203=0.00\n208=0");
        let tree = await editDraftNutrient(harness, "203");
        const label = available
          ? "Protein amount (g per 100 g)"
          : "Nutrient ID 203 amount (per 100 g; unit unavailable)";
        expect(input(tree, label).props.editable).toBe(true);
        await type(harness, label, "12.340006");
        tree = await click(harness, applyLabel);
        expect(canonical(tree)).toBe("203=12.340006\n208=0");
        tree = await editDraftNutrient(harness, "203");
        tree = await type(harness, label, "0.000001");
        const apply = button(tree, applyLabel).props.onPress;
        const field = input(tree, label).props.onChangeText;
        available = !available;
        tree = await click(harness, "Refresh private data");
        expect(button(tree, applyLabel).props.disabled).toBe(true);
        expect(text(tree)).toContain(stale);
        ...
        button(tree, applyLabel).props.onPress();
        expect(harness.stateWrites).toBe(stateWrites);
        const currentLabel = available
          ? "Protein amount (g per 100 g)"
          : "Nutrient ID 203 amount (per 100 g; unit unavailable)";
        expect(input(tree, currentLabel).props.value).toBe("0.000001");
        await click(harness, "Clear nutrient entry");
        await editDraftNutrient(harness, "203");
        await click(harness, "Trace");
        tree = await click(harness, applyLabel);
        expect(canonical(tree)).toBe("203=trace\n208=0");
        expect(writes(requests)).toHaveLength(0);
      } finally {
        harness.unmount();
      }
  ...
  it("retains failed Apply and exact state/reason input until a valid explicit update", async () => {
    const { harness, requests } = setup();
    try {
      await fillManual(harness);
      await type(harness, "Canonical nutrients per 100 g", "208=0\n203=0.00\r\n");
      await editDraftNutrient(harness, "203");
      await type(harness, "Protein amount (g per 100 g)", " invalid raw ");
      let tree = await click(harness, applyLabel);
      expect(canonical(tree)).toBe("208=0\n203=0.00\r\n");
      expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe(" invalid raw ");
      await click(harness, "Unknown");
      tree = await click(harness, applyLabel);
      expect(text(tree)).toContain("Choose an explicit unknown reason before applying this edit.");
      await click(harness, "Withheld");
      tree = await click(harness, applyLabel);
      expect(canonical(tree)).toBe("208=0\n203=unknown:withheld\r\n");
      await editDraftNutrient(harness, "203");
      expect(button(await harness.settle(), "Withheld").props.accessibilityState.selected).toBe(
        true,
      );
      await click(harness, "Trace");
      tree = await click(harness, applyLabel);
      ...
      await editDraftNutrient(harness, "203");
      await click(harness, "Quantified");
      await type(harness, "Protein amount (g per 100 g)", "0.00");
      tree = await click(harness, applyLabel);
      expect(canonical(tree)).toBe("208=0\n203=0.00\r\n");
      expect(writes(requests)).toHaveLength(0);
    } finally {
  ...
  for (const boundary of ["owner", "background", "replay"])
    it(`fences retained edit/apply through ${boundary} and preserves current recovery`, async () => {
      const { harness, requests } = setup();
      try {
        await fillManual(harness);
        await type(harness, "Canonical nutrients per 100 g", "208=0\n203=0.00");
        ...
        expect(harness.stateWrites).toBe(stateWrites);
        expect(requests).toHaveLength(count);
        if (boundary === "owner") harness.flushEffects();
        else if (boundary === "background") state("active");
        tree = await harness.settle();
        if (boundary === "owner") {
          expect(button(tree, "Add nutrient row to draft")).toBeDefined();
          expect(input(tree, draftFilterLabel).props.value).toBe("");
...
function namedComposerSection(tree) {
  const matches = nodes(
    tree,
    (node) =>
      node.type === "View" &&
      React.Children.toArray(node.props.children).some(
        (child) =>
          child.type === "Text" &&
          ["Add a named nutrient row", "Edit nutrient row"].includes(text(child)),
      ),
  );
  expect(matches).toHaveLength(1);
  return matches[0];
}
function namedComposerChoice(tree, id) {
  const matches = nodes(
    namedComposerSection(tree),
    (node) => node.props.accessibilityRole === "radio" && node.key === id,
  );
  expect(matches).toHaveLength(1);
  return matches[0];
}

describe("native Add-picker nutrient availability", () => {
  it("keeps loaded source order and duplicate names independent by exact ID while disabling occupied alternatives", async () => {
    const definitions = [
      { ...protein, id: "9007199254740993", code: "large_id", name: "Shared nutrient", unit: "mg" },
      { ...sodium, name: "Shared nutrient" },
      protein,
    ];
    const { harness, requests, props } = setup((request) =>
      request.url.pathname === "/v1/nutrients/targetable"
    ...
    try {
      const raw = " 307=trace\r\n999=unknown:not_reported ";
      await type(harness, "Canonical nutrients per 100 g", raw);
      let tree = await type(harness, "Find an available nutrient by name", "  SHARED  ");
      const choices = nodes(
        namedComposerSection(tree),
        (node) =>
          node.props.accessibilityRole === "radio" &&
          ["307", "9007199254740993"].includes(node.key),
      );
      expect(
        choices.map((choice) => [choice.key, text(choice), choice.props.accessibilityState]),
      ).toEqual([
        ["9007199254740993", "Shared nutrient (mg)", { selected: false, disabled: false }],
        [
          "307",
          "Shared nutrient (mg) · already in this draft",
          { selected: false, disabled: true },
        ],
      ]);
      expect(text(namedComposerSection(tree))).toContain(
        "2 matching of 3 loaded nutrients. 2 unused loaded nutrients.",
      );
      const fields = rawCustomFields(tree);
      const count = requests.length;
      const operation = hooks.operation;
      const stateWrites = harness.stateWrites;
      namedComposerChoice(tree, "307").props.onPress();
      button(tree, "Add nutrient row to draft").props.onPress();
      expect(harness.stateWrites).toBe(stateWrites);
      namedComposerChoice(tree, "9007199254740993").props.onPress();
      tree = await harness.settle();
      expect(namedComposerChoice(tree, "9007199254740993").props.accessibilityState.selected).toBe(
        true,
      );
      await type(harness, "Shared nutrient amount (mg per 100 g)", "0.00000000000000100");
      tree = await type(harness, "Find an available nutrient by name", "no match");
      expect(text(namedComposerSection(tree))).toContain(
        "0 matching of 3 loaded nutrients. 2 unused loaded nutrients.",
      );
      expect(text(namedComposerSection(tree))).not.toContain("All loaded nutrients are already");
      tree = await click(harness, "Clear nutrient filter");
      expect(
        ["9007199254740993", "307", "203"].map((id) => namedComposerChoice(tree, id).key),
      ).toEqual(definitions.map((item) => item.id));
      expect(rawCustomFields(tree)).toEqual(fields);
      expect(requests).toHaveLength(count);
      expect(hooks.operation).toBe(operation);
      expect(props.quickAddOutboxController.enqueueOperation).not.toHaveBeenCalled();
  ...
  it("retains occupied selection, exact scratch, filters and pending choice until removal releases that ID, then preserves acknowledged Save retry", async () => {
    let attempt = 0;
    const { harness, requests } = setup((request) =>
      request.method === "POST"
        ? ++attempt === 1
    ...
    try {
      ...
      const raw = " \t203=0.00 \r\n307=trace\n";
      await type(harness, "Canonical nutrients per 100 g", raw);
      await type(harness, "Find an available nutrient by name", " ");
      await type(harness, draftFilterLabel, "203");
      let tree = await copyCustom(harness);
      const fields = rawCustomFields(tree);
      const count = requests.length;
      const operation = hooks.operation;
      const stateWrites = harness.stateWrites;
      expect(text(namedComposerSection(tree))).toContain("0 unused loaded nutrients.");
      expect(text(namedComposerSection(tree))).toContain(
        "All loaded nutrients are already in this draft.",
      );
      expect(namedComposerChoice(tree, "203").props.accessibilityState).toEqual({
        selected: true,
        disabled: true,
      });
      namedComposerChoice(tree, "203").props.onPress();
      namedComposerChoice(tree, "307").props.onPress();
      button(tree, "Add nutrient row to draft").props.onPress();
      expect(harness.stateWrites).toBe(stateWrites);
      tree = await harness.settle();
      ...
      expect(namedComposerChoice(tree, "307").props.disabled).toBe(true);
      expect(text(namedComposerSection(tree))).toContain("1 unused loaded nutrient.");
      tree = await click(harness, "Quantified");
      expect(input(tree, "Protein amount (g per 100 g)").props.value).toBe("0.00000000000000100");
      await click(harness, "Unknown");
      tree = await click(harness, "Add nutrient row to draft");
      expect(canonical(tree)).toBe("\r\n307=trace\n203=unknown:withheld");
      expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
      expect(namedComposerChoice(tree, "203").props.accessibilityState.selected).toBe(true);
      const afterAddWrites = harness.stateWrites;
      button(tree, "Add nutrient row to draft").props.onPress();
      namedComposerChoice(tree, "203").props.onPress();
      expect(harness.stateWrites).toBe(afterAddWrites);
      expect(requests).toHaveLength(count);
      expect(hooks.operation).toBe(operation);
      await click(harness, "Create private food");
      const first = writes(requests)[0];
      expect(JSON.parse(first.body).nutrients).toEqual([
        { nutrientId: "307", state: "trace", amountPer100Grams: null },
        { nutrientId: "203", state: "unknown", amountPer100Grams: null, reason: "withheld" },
      ]);
      tree = await click(harness, "Create private food");
      expect(writes(requests)).toHaveLength(2);
      expect(writes(requests)[1].body).toBe(first.body);
  ...
  it("keeps unknown occupancy separate from empty, exhausted, capacity and unavailable metadata without repairing raw text", async () => {
    let phase = "ready";
    const { harness, requests } = setup((request) =>
      request.url.pathname === "/v1/nutrients/targetable"
        ? phase === "failed"
          ? response({}, 503)
          : response({ data: phase === "empty" ? [] : [protein, sodium] })
        : undefined,
    );
    try {
      let tree = await harness.settle();
      expect(text(namedComposerSection(tree))).toContain("2 unused loaded nutrients.");
      for (const raw of ["203=0\n203=trace", "203=0\ninvalid row"]) {
        tree = await type(harness, "Canonical nutrients per 100 g", raw);
        expect(text(namedComposerSection(tree))).toContain(
          "Fix invalid or duplicate canonical nutrient text below before adding a named row.",
        );
        expect(text(namedComposerSection(tree))).not.toContain("unused loaded");
        expect(text(namedComposerSection(tree))).not.toContain("already in this draft");
        expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
        const stateWrites = harness.stateWrites;
        namedComposerChoice(tree, "203").props.onPress();
        button(tree, "Add nutrient row to draft").props.onPress();
      ...
      await click(harness, "Protein (g)");
      await click(harness, "Trace");
      const full = Array.from({ length: 256 }, (_, index) => `${index + 1000}=0`).join("\n");
      tree = await type(harness, "Canonical nutrients per 100 g", full);
      expect(text(namedComposerSection(tree))).toContain("2 unused loaded nutrients.");
      tree = await click(harness, "Add nutrient row to draft");
      expect(canonical(tree)).toBe(full);
      expect(text(namedComposerSection(tree))).toContain("256");
      await type(harness, "Canonical nutrients per 100 g", "203=0\n307=trace");
      phase = "empty";
      tree = await click(harness, "Refresh private data");
      expect(text(namedComposerSection(tree))).toContain(
      ...
      phase = "failed";
      tree = await click(harness, "Refresh private data");
      expect(text(namedComposerSection(tree))).toContain("The named nutrient list has not loaded.");
      expect(text(namedComposerSection(tree))).not.toContain("unused loaded");
      expect(canonical(tree)).toBe("203=0\n307=trace");
      expect(writes(requests)).toHaveLength(0);
    } finally {
  ...
  it("rejects retained choice/Add after raw ABA and metadata replacement while current free choice and fixed-row Apply remain usable", async () => {
    let replaced = false;
    const { harness, requests } = setup((request) =>
      replaced && request.url.pathname === "/v1/nutrients/targetable"
        ? response({ data: [{ ...protein, name: "Current protein" }] })
        : undefined,
    );
    try {
      await fillManual(harness);
      await click(harness, "Protein (g)");
      let tree = await type(harness, "Protein amount (g per 100 g)", "0.00000000000000100");
      const oldChoice = namedComposerChoice(tree, "307").props.onPress;
      const oldAdd = button(tree, "Add nutrient row to draft").props.onPress;
      input(tree, "Canonical nutrients per 100 g").props.onChangeText("208=0\n307=trace");
      tree = harness.renderWithoutEffects();
      input(tree, "Canonical nutrients per 100 g").props.onChangeText("208=0");
      tree = harness.renderWithoutEffects();
      let stateWrites = harness.stateWrites;
      ...
      namedComposerChoice(tree, "307").props.onPress();
      tree = await harness.settle();
      const removedChoice = namedComposerChoice(tree, "307").props.onPress;
      const removedAdd = button(tree, "Add nutrient row to draft").props.onPress;
      replaced = true;
      tree = await click(harness, "Refresh private data");
      expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
      expect(input(tree, "Exact amount per 100 g").props.value).toBe("0.00000000000000100");
      ...
      expect(harness.stateWrites).toBe(stateWrites);
      expect(requests).toHaveLength(count);
      tree = await click(harness, "Current protein (g)");
      tree = await click(harness, "Add nutrient row to draft");
      const raw = canonical(tree);
      expect(raw).toBe("208=0\n203=0.00000000000000100");
      expect(button(tree, "Add nutrient row to draft").props.disabled).toBe(true);
      tree = await editDraftNutrient(harness, "203");
      expect(text(namedComposerChoice(tree, "203"))).toBe("Current protein (g)");
      expect(button(tree, "Apply nutrient edit").props.disabled).toBe(false);
      tree = await click(harness, "Apply nutrient edit");
      expect(canonical(tree)).toBe(raw);
      expect(requests).toHaveLength(count);
      expect(writes(requests)).toHaveLength(0);
    } finally {
