...
import {
  ...
  type FdcStagingPage,
  openVerifiedFdcRecordExport,
  postgresJsonTextByteUpperBound,
  type VerifiedFdcRecordExport,
} from "../src/fdc-record-reader.js";
...
const roots: string[] = [];
const readers: VerifiedFdcRecordExport[] = [];
...
describe("fully verified bounded FDC record reader", () => {
  ...
  it("resumes at deterministic checkpoints and replays only the preceding page when requested", async () => {
    ...
    expect(
      (await collect(reader.pages({ nextOffset: 500, replayPreviousPage: true }))).map(
        (page) => page.expectedNextOffset,
      ),
    ).toEqual([250, 500]);
    expect(
      (await collect(reader.pages({ nextOffset: 501, replayPreviousPage: true }))).map(
        (page) => page.expectedNextOffset,
      ),
    ).toEqual([500]);
    expect(await collect(reader.pages({ nextOffset: 501 }))).toEqual([]);
    for (const offset of [-1, 1, 249, 251, 502, 1.5, Number.NaN])
      expect(() => reader.pages({ nextOffset: offset })).toThrow("deterministic page boundary");
  });
  ...
  it("uses encoded byte limits to split large rows before 250 records", async () => {
    ...
    expect(
      (await collect(reader.pages({ nextOffset: checkpoint, replayPreviousPage: true })))[0]
        ?.nextOffset,
    ).toBe(checkpoint);
  });
  ...
  it.each(["hash", "size"])(
    ...
    async (kind) => {
      ...
      await expect(openVerifiedFdcRecordExport(fixture)).rejects.toThrow();
  ...
  it("rejects source identity mismatch and unsupported PostgreSQL text", async () => {
    for (const description of ["nul\0text", "unpaired\ud800"]) {
      ...
      await expect(openVerifiedFdcRecordExport(fixture)).rejects.toThrow("NUL or unpaired");
      expect(fixture.verifyInspection).not.toHaveBeenCalled();
    }
    const fixture = await createFixture([
      { ...record(0), source: { ...record(0).source, releaseKey: "different" } },
    ]);
    await expect(openVerifiedFdcRecordExport(fixture)).rejects.toThrow("identity differs");
  });

  it("rejects counts above 10000 and records above 1 MiB", async () => {
    const countFixture = await createFixture(records(0));
    countFixture.expectedBaseline.parserBaselineCsvAcceptedFoodCount = 10_001;
    await expect(openVerifiedFdcRecordExport(countFixture)).rejects.toThrow("10000-record cap");
    const sizeFixture = await createFixture(records(1, "x".repeat(1_048_576)));
    await expect(openVerifiedFdcRecordExport(sizeFixture)).rejects.toThrow(
  ...
  it("preflights the conservative PostgreSQL whole-batch cap before acceptance", async () => {
    ...
    await expect(openVerifiedFdcRecordExport(fixture)).rejects.toThrow(
      "64-MiB PostgreSQL payload bound",
    );
    expect(fixture.verifyInspection).not.toHaveBeenCalled();
  });
  ...
  it("rejects parent symlinks and unsafe input namespaces", async () => {
    ...
    for (const inputPath of [
      ...
      await expect(openVerifiedFdcRecordExport({ ...fixture, inputPath })).rejects.toThrow(
        "canonical private Linux",
      );
    await rename(fixture.parent, `${fixture.parent}-real`);
    await symlink(`${fixture.parent}-real`, fixture.parent);
    await expect(openVerifiedFdcRecordExport(fixture)).rejects.toThrow("private directories");
  });
  ...
  it.each(["source", "parent"])(
    ...
    async (target) => {
      ...
      await expect(openVerifiedFdcRecordExport(fixture)).rejects.toThrow("identity changed");
      expect(
        await readFile(
  ...
  it("honors cancellation during copying and paging", async () => {
    ...
    await expect(
      openVerifiedFdcRecordExport({ ...fixture, signal: controller.signal }),
    ).rejects.toThrow("copy cancelled");
    expect(fixture.verifyInspection).not.toHaveBeenCalled();
    hooks.beforeRead = undefined;
    const second = new AbortController();
    const reader = await openVerifiedFdcRecordExport({ ...fixture, signal: second.signal });
    readers.push(reader);
...
async function accept(fixture: Fixture) {
  const reader = await openVerifiedFdcRecordExport(fixture);
  readers.push(reader);
  return reader;
}
