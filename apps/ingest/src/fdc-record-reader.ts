...
const MAX_POSTGRES_BYTES = 67_108_864;
const MAX_PAGE_BYTES = 16_777_216;
const MAX_LINE_BYTES = 15_728_640;
const MAX_EXPORT_BYTES = MAX_POSTGRES_BYTES + MAX_LINE_BYTES + 65_536 + MAX_RECORDS + 2;
...
export interface VerifiedFdcRecordExport {
  readonly evidence: {
...
/** Verifies the entire export into an anonymous private snapshot before exposing any staging page. */
export async function openVerifiedFdcRecordExport(input: {
...
}): Promise<VerifiedFdcRecordExport> {
  const { inputPath, workspaceRoot, expectedExport, expectedHeader, expectedBaseline, signal } =
    input;
  ...
  if (
    ...
    expectedExport.byteSize > MAX_EXPORT_BYTES
  ) {
  ...
  if (
    ...
    throw new Error(
      "FDC record export requires the complete reviewed baseline within the 10000-record cap",
    );
  }
  ...
  try {
    ...
    let pageCount = 0;
    let pageBytes = EMPTY_PAGE_BYTES;
    ...
    for await (const line of lines(source, signal, async (chunk) => {
      ...
      const record = stagingRecord(value, recordCount, line.length, expectedHeader);
      recordCount += 1;
      if (recordCount > expectedCount || recordCount > MAX_RECORDS)
        throw new Error(
          "FDC record export record count exceeds its reviewed baseline or 10000-record cap",
        );
      ...
      if (postgresPayloadByteUpperBound > MAX_POSTGRES_BYTES)
        throw new Error(
          "FDC record export exceeds the conservative 64-MiB PostgreSQL payload bound",
        );
      const entryBytes = stageEntryBytes(record);
      if (pageCount === 250 || pageBytes + entryBytes + (pageCount ? 1 : 0) > MAX_PAGE_BYTES) {
        pageEnds.push(recordCount - 1);
        pageCount = 0;
        pageBytes = EMPTY_PAGE_BYTES;
      }
      pageBytes += entryBytes + (pageCount ? 1 : 0);
      pageCount += 1;
      if (pageBytes > MAX_PAGE_BYTES)
        throw new Error("FDC record export record exceeds the staging request budget");
    }
    if (pageCount) pageEnds.push(recordCount);
    ...
    return {
      ...
      pages({ nextOffset, replayPreviousPage = false }) {
        ...
        if (
          ...
          (nextOffset !== 0 && !pageEnds.includes(nextOffset))
        )
          throw new Error("FDC staging checkpoint is not a verified deterministic page boundary");
        const boundary = nextOffset === 0 ? -1 : pageEnds.indexOf(nextOffset);
        const startOffset =
          replayPreviousPage && boundary >= 0 ? (pageEnds[boundary - 1] ?? 0) : nextOffset;
        active = true;
        return (async function* (): AsyncGenerator<FdcStagingPage> {
          try {
            let sequence = 0;
            let pageStart = startOffset;
            ...
            for await (const line of lines(snapshotReader, signal)) {
              ...
              sequence += 1;
              if (sequence <= startOffset) continue;
              documentBytes += stageEntryBytes(row) + (records.length ? 1 : 0);
              records.push(row);
              if (pageEnds.includes(sequence)) {
                signal?.throwIfAborted();
                yield Object.freeze({
            ...
            if (sequence !== recordCount || records.length)
              throw new Error("FDC record snapshot paging did not match its verified boundaries");
          } finally {
            active = false;
          }
        })();
