...
function readyFetcher() {
  const fetcher = vi.fn(async (url: string, _init?: RequestInit): Promise<Response> => {
    if (url === "/api/auth/me") return session(props.ownerUserId);
...
describe("actual pasted ingredient review lifecycle", () => {
  ...
  it("invalidates an earlier confirmation when that line is reopened for a new choice", async () => {
    ...
    invoke(button("Add reviewed ingredients"), "onClick");
    await hooks.settle();
  ...
  it("rejects retained search/result callbacks after the current query or line changes", async () => {
    ...
    invoke(staleSearch, "onClick");
    await hooks.settle();
    expect(fetcher).not.toHaveBeenCalled();
    await click("Search foods");
    const staleSelection = button("Select Rolled oats");
    await click("Review line 2");
    invoke(staleSelection, "onClick");
    await hooks.settle();
  ...
  it.each(["owner", "disabled", "unmount"] as const)(
    ...
    async (transition) => {
      ...
      expect(hasButton("Select Rolled oats")).toBe(false);
      if (transition === "owner") expect(field("Pasted ingredient lines").props.value).toBe("");
  ...
  it.each(["owner", "disabled", "unmount", "capacity"] as const)(
